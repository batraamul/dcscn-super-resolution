...
class SuperResolution(tf_graph.TensorflowGraph):
    ...
    def get_model_name(self, model_name, name_postfix=""):
        print("get_model_name directory in dcscn is called")
        if model_name is "":
    ...
    def load_dynamic_datasets(self, data_dir, batch_image_size):
        ...
        """
        print("load_dynamic_datasets of DCSCN is called")
    ...
    def load_datasets(self, data_dir, batch_dir, batch_image_size, stride_size=0):
        ...
        """
        print("load_datasets of DCSCN is called")
    ...
    def init_epoch_index(self):
        print("init_epoch_index of DCSCN is called")
    ...
    def build_input_batch(self):
        print("build_input_batch of DCSCN is called")
    ...
    def load_graph(self, frozen_graph_filename='./model_to_freeze/frozen_model_optimized.pb'):
        ...
        """
        print("load_graph in DCSCN is called")
    ...
    def build_graph(self):
        print("build_graph in DCSCN is called")
        print("The no of channels are ",self.channels)
        ...
        for i in range(self.layers):
            ...
            print("THE INPUT TENSOR IS_INITIALLY" ,input_tensor)
            if (self.depthwise_separable):
                self.build_depthwise_separable_conv("DEPTHWISE_SEPRABLE_CNN%d" % (i + 1), input_tensor, self.cnn_size, input_feature_num,
                                            output_feature_num, use_bias=True, activator=self.activator,
                                            use_batch_norm=self.batch_norm, dropout_rate=self.dropout_rate)
            else:
                self.build_conv("NORMAL_2_CNN%d" % (i + 1), input_tensor, self.cnn_size, input_feature_num,
                                output_feature_num, use_bias=True, activator=self.activator,
                                use_batch_norm=self.batch_norm, dropout_rate=self.dropout_rate)
